...
var BuiltInFunctions = []BuiltInFunction{
	...
	},
	{
		...
		Description: "Extracts a JSON string and converts it to a SQL `STRING` value.Arguments:`json_expr`: JSON. For example:\n\n```\nJSON '{\"name\": \"sky\", \"color\": \"blue\"}'\n\n```\n\nIf this expression is SQL `NULL`, the function returns SQL\n`NULL`. If the extracted JSON value is not a string, an error is produced.",
		ExampleSQLs: []string{
	...
	{
		Name:        "SEARCH",
		Method:      "SEARCH(search_data, search_query[, json_scope=>json_scope_value][, analyzer=>analyzer_name])\n\njson_scope_value:\n  { 'JSON_VALUES' | 'JSON_KEYS' | 'JSON_KEYS_AND_VALUES' }\n\nanalyzer_name:\n  { 'LOG_ANALYZER' | 'NO_OP_ANALYZER' }",
		Description: "The `SEARCH` function checks to see whether a BigQuery table or other\nsearch data contains a set of search terms. It returns `TRUE` if all tokens in\nthe `search_query` appear in the `search_data` based on the tokenization\ndescribed in the [text analyzer](#text_analyzer), and `FALSE` otherwise. If\n`search_query` is `NULL` or doesn't contain any search terms, an error is\nthrown.`SEARCH` is designed to work with\n[search indexes](/bigquery/docs/search-index) to\noptimize point lookups. Although the `SEARCH` function works for columns of a\ntable that are not indexed, its performance will be greatly improved with a\nsearch index.",
		ExampleSQLs: []string{},
