...
var templateCode = `// Code generated by gen_builtin_function.go, DO NOT EDIT.
package function
...
func loadFunctions(url string) []function {
	...
	doc.Find("div.devsite-article-body").Find("tbody").Children().Each(func(i int, s *goquery.Selection) {
		if method, ok := s.Find("a").Attr("href"); ok {
	...
	for _, method := range methods {
		document := doc.Find(fmt.Sprintf("h3%s", method))
		if !document.Is("h3") {
			continue
		}
		name := document.Text()
		functionURL := fmt.Sprintf("%s%s", url, method)

		methodSelection := document.Next()
		method := strings.TrimSpace(methodSelection.Text())

		descriptionSelection := methodSelection
		i := 0
		for !descriptionSelection.Is("h3") && i < 10 {
		...
		i = 0
		for !exampleSelection.Is("h3") && i < 10 {
			if exampleSelection.Text() == "Examples" {
				break
			}
